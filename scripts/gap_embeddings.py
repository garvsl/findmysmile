...
#load the data
gap = SimpleDirectoryReader(input_dir="../data/gap").load_data() #read the content within the crossbite directory

 # only read one file for one procedure
print("raw data:")
print(gap)  # print out the data to see what it looks like raw

#index creation
index = VectorStoreIndex.from_documents(gap)  # embed the pdf content
...
txt_file_path = os.path.join(output_dir, "gap_query_response.txt")
